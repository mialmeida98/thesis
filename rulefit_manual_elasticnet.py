...
### B. IMPLEMENT ELASTICNET ###
regr = ElasticNet(random_state=0)
regr.fit(X_train, y_train)
ElasticNet(random_state=0)
print('Coefficients: ',regr.coef_)
print('Intercept: ',regr.intercept_)
print('Predictions: ',regr.predict(X_test[:5]))
print('MAE: ',mean_absolute_error(regr.predict(X_test[:5]), y_test[:5]))
print('MSE: ',mean_squared_error(regr.predict(X_test[:5]), y_test[:5]))
print('RMSE: ',np.sqrt(mean_squared_error(regr.predict(X_test[:5]), y_test[:5])))
